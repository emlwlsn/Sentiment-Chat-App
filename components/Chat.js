...
class Chat extends Component {
    ...
    // initializing state to contain empty chats array
    state = { chats: [] }

    // setting up Pusher connection
    componentDidMount() {
        const appId = process.env.PUSHER_APP_KEY;
        const cluster = process.env.PUSHER_APP_CLUSTER;

        // creates new pusher object with the appropriate values
        this.pusher = new Pusher(appId, {
            cluster: cluster,
            encrypted: true
        });
        
        // subscribes to chat room through pusher
        this.channel = this.pusher.subscribe('chat-room');
        
        // sends messages to chats array
    ...
    // event handler to send message and timestamp
    handleKeyUp = evt => {
    ...
    return (this.props.activeUser && <Fragment>
        ...
        <div className="px-4 pb-4 w-100 d-flex flex-row flex-wrap align-items-start align-content-start position-relative" style={{ height: 'calc(100% - 180px)', overflowY: 'scroll' }}>
            ...
            {/*going through all chats in chats array*/}
            {this.state.chats.map((chat, index) => {

                const previous = Math.max(0, index-1);
                const previousChat = this.state.chats[previous];
                // checks if the chat is from the active user to decide which side to put message
                ...
                // displays user's name and their message that they sent
                return (
                    <Fragment key={index}>
